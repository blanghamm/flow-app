...
const fetchAllTasks = async <T>(): Promise<T> => await fetch('http://localhost:3000/api/tasks', { method: 'get' }).then((response) => response.json() as Promise<T>)
const { data, suspense } = useQuery({ queryKey: ['tasks'], queryFn: fetchAllTasks<Task[]> })
await suspense()

const updatedTask = ref<Task | {}>({})

const postUpdatedTask = async <T>(): Promise<T> => await fetch('http://localhost:3000/api/tasks', { method: 'post', body: JSON.stringify(updatedTask.value) }).then((response) => response.json() as Promise<T>)
